...
To evaluate the framework three example applications were developed using Kotlin Remote and then
rewritten using two alternative frameworks: gRPC and Kotlin RPC. All three implementations of each
application are functionally equivalent, the only difference is in the way network interactions are
organized. As we are interested in network code, we will be focusing on it during comparison.
Business logic of applications (database access, command line user interface, etc.) is roughly the
same in all three implementations. Source code of all the applications is available at
\texttt{integration-tests/} folder of Kotlin Remote repository.

Three example applications were chosen so that they cover different scenarios in which RPC framework
can be used. The first one is a simple Todo application with a small fixed set of CRUD operations.
The second one is a chat with rooms in which users send messages to each other through a central
server. The third one is a small bank application which is split into three microservices that talk
to each other. These applications differ in size and complexity, and stress different aspects of
RPC frameworks.
...
The third application is the largest. It is a small banking system split into three microservices:
\kotlini{UserService}, \kotlini{AccountService} and \kotlini{PaymentService}. User service handles
registration and login, account service handles accounts and transfers, payment service records
payments and uses account service for transfers. Each microservice runs on its own port. Client is
a simple command line tool that talks to all three services.

The bank application illustrates how Kotlin Remote handles service-to-service calls. Listing
\ref{bank_payment_service} shows \kotlini{PaymentServiceImpl.makePayment} function. Notice how
context is switched from \kotlini{PaymentServiceConfig} to \kotlini{AccountServiceConfig} when call
to \kotlini{accountService.transfer} is made. This is enough to redirect the call to the account
microservice.
...
\begin{listing}[!h]
  ...
  \end{minted}
  \caption{Service-to-service call in the bank application}\label{bank_payment_service}
\end{listing}

It is worth noticing that \kotlini{PaymentServiceImpl} itself is a remote class. Reference to
\kotlini{accountService} that is held inside \kotlini{PaymentServiceImpl} is also remote. In
Kotlin Remote remote references compose naturally, calls between services do not require any
additional plumbing besides switching the context to the target microservice configuration.

\subsection{Comparison to alternatives}

For each example application two alternative implementations were written, one using gRPC and one
using Kotlin RPC. Comparison was done on the size of network code and on the amount of work
required to add a new remote function or new microservice. We also looked at how each framework
handles service-to-service calls and at code style of the resulting applications.

\subsubsection{Lines of code}

Table \ref{loc_table} shows the size of network code in lines for all three applications. By
network code we mean code that exists only because the application is distributed: remote
function declarations and decorations, service interfaces, IDL files, client and server setup,
type conversions between domain types and IDL types. Pure business logic (repositories, command
line interface, desktop UI, database schema) and pure data classes are excluded. Numbers should
be taken as rough approximations because in Kotlin Remote network annotations are interleaved
with business logic and exact split is somewhat subjective.

\begin{table}[!h]
  \begin{center}
    \begin{tabular}{lrrr}
      \toprule
      Application & Kotlin Remote & gRPC & Kotlin RPC \\
      \midrule
      Todo (4 remote functions) & 164 & 110 & 86 \\
      Rooms chat$^{\dagger}$ & ${\sim}165$ & ${\sim}150$ & ${\sim}120$ \\
      Bank (3 microservices) & ${\sim}280$ & ${\sim}390$ & ${\sim}330$ \\
      \bottomrule
    \end{tabular}
    \caption{Lines of network code per application. $^{\dagger}$Rooms chat numbers are not
    directly comparable, see discussion below.}\label{loc_table}
  \end{center}
\end{table}

Numbers are informative. For the smallest application (Todo) Kotlin Remote produces noticeably
more network code than the alternatives. The reason is that Kotlin Remote requires per application
setup of \kotlini{RemoteClient} and \kotlini{RemoteServer} which is around 100 lines for any
application regardless of its size. Kotlin RPC and gRPC have leaner per application setup, but
require to write some code per remote function (interface declaration, service implementation
and in case of gRPC also IDL file and proto-to-domain mapping).

For larger applications situation changes. In the bank application Kotlin Remote produces the
smallest network code, although the gap with Kotlin RPC is not huge. Per microservice overhead
in Kotlin Remote is small: a single \kotlini{RemoteConfig} object of seven lines per microservice
plus a launcher of around 15 lines. The shared server setup code in \kotlini{RemoteServer.kt} is
written once and used by all three microservices. gRPC pays a high price per microservice because
of separate proto files and proto-to-domain conversion code. Kotlin RPC sits in between: it does
not need IDL but still requires service interface and implementation class per microservice.

The rooms chat row needs a separate explanation because the three implementations are not exactly
the same application. In Kotlin Remote version each user runs its own embedded server in addition
to the central one, and message delivery is done as a remote call from central server to user
machine (peer-to-peer). In gRPC and Kotlin RPC versions central server keeps a per-user mailbox
and pushes messages over a server side stream (\kotlini{stream ChatMessage} in gRPC and
\kotlini{Flow<ChatMessage>} in Kotlin RPC). So Kotlin Remote version effectively does more: every
participant is both client and server, while in alternatives only the central node is a server.
This shows up in the line count: Kotlin Remote pays for setting up an embedded server on each
participant. If we discount this extra cost, Kotlin Remote rooms chat would be roughly comparable
to Kotlin RPC version. The opposite reading is also valid: Kotlin Remote forced us into the
peer-to-peer design because it does not support streaming (see section
\ref{streaming_limitation_section}), and the extra lines are the cost of working around this
limitation. Either way, rooms chat numbers should not be read as direct apples-to-apples
comparison, they only show that Kotlin Remote stays in the same order of magnitude as
alternatives even when used in a way it was not optimized for.

\subsubsection{Per-function overhead}

Another way to compare frameworks is to count the amount of work needed to add a new remote
...
Listing \ref{per_function_compare} shows side by side what has to be written for the same function
in each framework.

\begin{listing}[!h]
  \begin{minted}{kotlin}
  // Kotlin Remote: 3 lines
  @Remote
  context(_: RemoteContext<ServerConfig>)
  ...
  // gRPC: rpc and message in .proto + override in impl class + proto/domain mapping
  // todoapp.proto:
  // message CreateTodoRequest { string title = 1; }
  // service TodoService { rpc CreateTodo(CreateTodoRequest) returns (Todo); }
  class TodoServiceImpl(...) : TodoServiceGrpcKt.TodoServiceCoroutineImplBase() {
      override suspend fun createTodo(request: CreateTodoRequest): Todo =
          repository.create(request.title)
  }
  \end{minted}
  \caption{Code required for one remote function}\label{per_function_compare}
\end{listing}

In Kotlin Remote remote function is added as an isolated unit, no other file has to be modified.
In Kotlin RPC each remote function has to be added in two places: once in service interface and
once in implementation class. In gRPC each remote function requires three places to be modified:
IDL file, implementation class and (often) mapping function between proto and domain types.

\subsubsection{gRPC}

gRPC clearly has the largest per function overhead among the three frameworks. The main source of
this overhead is the IDL: protobuf messages have to be defined for every request and response and
mapping functions between them and Kotlin domain types have to be written. In our bank application
we ended up with three separate proto files (\texttt{user.proto}, \texttt{account.proto},
\texttt{payment.proto}) totaling 108 lines just for IDL definitions, plus mapping code embedded
into repositories.

There is also a more subtle problem: protobuf types are not normal Kotlin classes. They are
generated builders that work via \kotlini{Foo.newBuilder().setX(...).build()} pattern. Code that
uses these types looks alien comparing to the rest of Kotlin code. In bank-grpc client every
remote call looks like
\kotlini{stub.makePayment(MakePaymentRequest.newBuilder().setAmount(...).build())} which is much
more verbose than equivalent Kotlin Remote call \kotlini{paymentService.makePayment(MakePaymentDto(amount, payer, payee))}.

On the positive side gRPC supports streaming out of the box. In the rooms chat application gRPC
implementation uses server side streaming (\kotlini{rpc Subscribe ... returns (stream ChatMessage)})
to push messages to subscribed users. This is a real feature missing from Kotlin Remote at the
moment. Of course it can be approximated with peer-to-peer remote calls as it is done in Kotlin
Remote rooms chat, but this requires running embedded server on every client which is not always
practical.

\subsubsection{Kotlin RPC}

Kotlin RPC is much closer to Kotlin Remote in spirit than gRPC. It uses Kotlin types directly
(no IDL), serialization is provided by kotlinx.serialization and remote functions are declared
as suspend functions. The main difference is that Kotlin RPC is service oriented: every remote
function has to be a method of an \kotlini{@Rpc} annotated interface. This forces user to declare
service interface and implementation class for every group of remote functions.

For applications that already have natural service decomposition (like our bank application)
Kotlin RPC overhead is moderate. For applications that do not have such decomposition (like our
Todo application) it adds extra layer of indirection without much benefit. Kotlin Remote in this
case allows just to write top level functions and mark them with \kotlini{@Remote}.

Kotlin RPC also supports streaming via Kotlin \kotlini{Flow}. This is a nice feature that fits
Kotlin ecosystem better than gRPC streams. Like in case of gRPC, this is something Kotlin Remote
does not support.

\subsection{Limitations}\label{limitations_section}

During development of the example applications several limitations of Kotlin Remote were
identified. Some of them are related to the early stage of the framework, others are inherent to
the chosen approach.

\subsubsection{Constant per application setup}

Kotlin Remote requires per application setup code that is roughly the same from one application to
another. \kotlini{RemoteClient.kt} (around 50 lines) and \kotlini{RemoteServer.kt} (around 60
lines) files in the Todo and bank applications are almost identical. This code configures
\kotlini{HttpClient}, \kotlini{ContentNegotiation} plugin, serialization modules for remote
functions and remote classes, installs \kotlini{KRemote} plugin and registers Ktor routes. Most of
this is parameter passing between framework components and could probably be hidden behind a more
opinionated default. For applications with single configuration this overhead dominates the
network code as can be seen in table \ref{loc_table} for the Todo application.

\subsubsection{Polymorphic serialization for remote interface types}

When remote class implements an interface and is used through that interface, polymorphic
serializer for the interface has to be registered manually. Listing
\ref{polymorphic_registration} shows code from the bank application that registers
\kotlini{PaymentService} and \kotlini{AccountService} interfaces with their stub implementations.
This is a standard kotlinx.serialization mechanism, but it is something user has to remember to
do.
...
\begin{listing}[!h]
  ...
  \end{minted}
  \caption{Polymorphic serializers registration in the bank application}\label{polymorphic_registration}
\end{listing}

In the future this could be done automatically by compiler plugin, because for each
\kotlini{@RemoteSerializable} class compiler already knows the list of interfaces it implements.

\subsubsection{No streaming support} \label{streaming_limitation_section}

Kotlin Remote currently supports only request-response style remote calls. There is no built in
way to return a stream of values from remote function or to push values from server to client. As
shown in the rooms chat application, this can be worked around using peer-to-peer remote calls,
but this is not always desirable. gRPC supports streaming through HTTP/2 streams and Kotlin RPC
supports streaming through Kotlin \kotlini{Flow}. Adding similar support to Kotlin Remote is one
of the directions for future work.

\subsubsection{Wire protocol coupling to source code}

Remote function in Kotlin Remote is identified on the wire by its fully qualified Kotlin name.
This means that if user renames remote function, moves it to another package or changes its
parameter types, all existing clients break. There is no versioning story like in protobuf where
fields have stable numbers and can be renamed without breaking the wire format. For applications
where client and server are in the same codebase this is not really a problem, because they are
always built together. For applications where client and server have separate release cycles this
can be a significant issue.

\subsubsection{No cross-language support}

By design both client and server should be written in Kotlin. This is a conscious trade-off,
because the framework relies on Kotlin compiler plugin to generate everything from source code.
gRPC supports many languages because it uses language agnostic IDL. For polyglot environments
gRPC is still a better choice.
...
writing. This means that user has to add \texttt{-Xcontext-parameters} compiler flag to enable it
and that the API of context parameters can change in future Kotlin releases. Kotlin Remote also
has to be updated for every new Kotlin compiler version because the compiler plugin API is not
stable.

\subsubsection{Multiple invocations of \kotlini{genCallableMap}}

Looking at \kotlini{RemoteClient.kt} and \kotlini{RemoteServer.kt} of any example application one
can notice that \kotlini{genCallableMap} is called several times. Each call is replaced at compile
time with the same map literal which can have hundreds of entries. As a result generated code is
larger than necessary. Caching of the result on user side is possible but inconvenient because
\kotlini{genCallableMap} has to be called from different scopes. Compiler plugin could
deduplicate these calls automatically but this is not implemented yet.
...
Currently every machine that runs Kotlin Remote application contains the full compiled artifact,
including code that is meant to be executed on other machines. For the bank application this
means that user microservice contains compiled code of account and payment microservices, even
though it never executes them. Type information about target machine is already encoded in remote
contexts (\kotlini{UserServiceConfig}, \kotlini{AccountServiceConfig}, etc.). This information
could be used by compiler plugin to slice the application into per node artifacts and include in
each artifact only code reachable from that node. This is essentially what tierless languages
discussed in section \ref{multi-tier-section} do at the language level. Doing it as a compiler
plugin pass over already typed Kotlin program is a feasible direction.
...
Several performance optimizations are possible. Default parameter values do not have to be
serialized and sent over the wire if remote side has the same defaults. Generated callable maps
could share common metadata between functions instead of duplicating it. Wire format could be
switched from JSON to a more compact binary format like Protocol Buffers or CBOR for cases where
wire size is important.

\subsubsection{Stub URL validation and security hardening}

Currently when stub of a remote class is sent from machine A to machine B and then to machine C,
machine C trusts the URL stored in the stub when it sends back lease requests or makes calls to
the original machine. There is no validation that the URL is legitimate and that the original
machine actually wanted to expose this stub to machine C. For trusted networks this is fine, but
for less trusted environments some form of stub signing or capability based access control would
be needed.
