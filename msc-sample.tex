...
this new framework \term{Kotlin Remote}. Kotlin Remote uses Kotlin's experimental \term{context
...
\begin{enumerate}
  ...
  \item Prototype support for distributed objects, that is classes with state that lives on
        remote machine.
  \item Implement and test the framework as a Kotlin compiler plugin together with a runtime
        library.
  \item Evaluate the result by comparing it with existing alternatives in terms of boilerplate
        code.
\end{enumerate}

Implementation of the framework should satisfy the following technical requirements:
\begin{enumerate}
  \item Any named Kotlin function should be callable remotely, not only methods of service
        interfaces. This includes top level functions, extension functions and methods of
        regular classes.
  \item Remote functions should be distinguishable from local ones at the type level, so
        developer always knows when network call is happening.
  \item Kotlin Multiplatform compatibility should be preserved, which means framework has to
        work on all KMP targets (JVM, JavaScript, Native, WebAssembly) without relying on
        reflection.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Section 5 concludes, discusses limitations and future work.
\end{enumerate}

\section{Background and Related work}

This section gives an overview of RPC as whole, as well as specific frameworks used in Kotlin
ecosystem to organize network interactions. Concepts used in this thesis, like coeffects and Kotlin
context parameters, are also explained here.

Before going further it is worth restating the scope. Existing frameworks discussed below (Ktor,
gRPC, Kotlin RPC) are general purpose tools that target a much wider scope than this work. They
are used in cross-language projects, in projects where client and server live in separate
codebases and have separate release cycles, and in projects where client and server are developed
by different teams. We do not claim that Kotlin Remote competes with them in those scenarios. On
the contrary, in those scenarios gRPC and Ktor are very likely the right choice. The discussion
below is about how these frameworks behave in shared-codebase Kotlin projects, which is the only
setting Kotlin Remote targets.

Proposed approach is a specific version of RPC. Because of that it makes sense to compare it mostly
to other RPC approaches used in Kotlin. The most popular ones are gRPC and new Kotlin RPC. These
frameworks are not boilerplate-heavy in general, their per function overhead is small. They do
however require developer to describe remote functions in a separate service interface and to
implement this interface in a service class. In shared-codebase setting both of these become
redundant, because client and server already share the codebase and the contract that the service
interface is supposed to encode is implicit in the shared code. Outside of shared-codebase setting
the service interface is not redundant, it is the explicit contract between independently
developed parts and existing frameworks are well-designed around it. Additionally, basically all
the existing RPC frameworks for Kotlin use Remote Method Invocation (RMI), the object-oriented
programming analog of RPC, where only methods of services can be executed remotely. Top level
functions cannot. This fits Kotlin only partially, because Kotlin is not entirely OOP language.
The main aim of developed framework is removal of the service interface redundancy in
shared-codebase Kotlin projects.

Apart from RPC frameworks, REST architectural style is also widely used in Kotlin to organize
network interactions. The most popular library for that is Ktor. Therefore, it is useful to take a
brief look at REST as well. Apart from looking at existing frameworks, this section also takes a
look on related ideas from functional programming. Namely on \term{coeffects}, that are
foundational concept on which Kotlin Remote is built. Coeffects are tightly related to
\term{implicit parameters}, which Kotlin recently got an implementation of in form of experimental
\term{context parameters} feature. Coeffects also have close ties to \term{multi-tier programming
languages}, in which separation between different machines is encoded in the type system.
Multi-tier languages are particularly relevant because they target precisely the shared-codebase
setting and inspired some of the design decisions made in Kotlin Remote. All of these ideas will
be discussed below.
...
separate codebases or cross-language communication). In those scenarios Kotlin Remote is simply not
applicable and other frameworks should be used. All implementations are functionally equivalent,
...
is available at \texttt{integration-tests/} folder of Kotlin Remote repository.
...
hierarchies between services in the type system. They differ on cross-language support, on the
choice of wire format and on whether the transport is fixed. Kotlin Remote differs from both
...
\begin{table}[!h]
  \begin{center}
    \begin{tabular}{lccc}
      ...
      Top level functions can be remote & No & No & Yes \\
      Remote call visible at call site & No & No & Yes \\
      ...
      Separate schema file & Yes (\texttt{.proto}) & No & No \\
      Transport & HTTP/2 (fixed) & Pluggable (Ktor) & Pluggable (Ktor) \\
      Wire format & Protocol Buffers & \texttt{kotlinx.serialization} & \texttt{kotlinx.serialization} \\
      \bottomrule
    \end{tabular}
...
\begin{listing}[!h]
  ...
  suspend fun getFeed(userId: Long): List<FeedItem> {
      ...
      return posts.map { p ->
          ...
          val likes = context(LikeServiceConfig.asContext()) { getPostLikes(p.id) }.size
          val comments = context(CommentServiceConfig.asContext()) { getPostComments(p.id) }.size
...
To call a function from another microservice it is enough to wrap the call into
\kotlini{context(XxxServiceConfig.asContext()) { ... }} block. There are no service stubs to
...
Client side benefits from context parameters as well. Listing \ref{social_client} shows the
test function that exercises the platform. It declares context parameters for all ten services
in a single \kotlini{context(...)} block on its signature. Inside the function every remote
call is a plain unqualified function call, no per-call context block and no service stub. In
Kotlin RPC version the same function takes ten service stubs as parameters and every call goes
through the corresponding stub.

\begin{listing}[!h]
  \begin{minted}{kotlin}
  context(
      _: RemoteContext<UserServiceConfig>, _: RemoteContext<PostServiceConfig>,
      _: RemoteContext<FollowServiceConfig>, _: RemoteContext<LikeServiceConfig>,
      _: RemoteContext<CommentServiceConfig>, _: RemoteContext<NotificationServiceConfig>,
      _: RemoteContext<FeedServiceConfig>, _: RemoteContext<SearchServiceConfig>,
      _: RemoteContext<WebBffConfig>, _: RemoteContext<MobileBffConfig>
  )
...
\begin{listing}[!h]
  ...
  suspend fun removeArticleAndComments(id: Long) {
      ...
      comments.forEach { deleteAnyComment(it.id) }          // requires ModeratorConfig
  ...
  suspend fun purgeUser(login: String): Int {
      ...
      articles.forEach { removeArticleAndComments(it.id) }  // requires ModeratorConfig
...
\subsection{Comparison to Kotlin RPC}
...
Pure business logic (function bodies, repositories, data classes, CLI text), Ktor transport
setup (\kotlini{HttpClient} and \kotlini{embeddedServer} construction, install of generic Ktor
plugins, routing wrappers, JSON serializer configuration), framework-specific transport-binding
helpers (\kotlini{install(KRemote)}, \kotlini{install(Krpc)}, \kotlini{rpc("/api")},
\kotlini{remote("/call")}, \kotlini{rpcConfig}, \kotlini{installKrpc()}), and imports, package
declarations and empty lines are all excluded. Numbers are approximations because the split
of some borderline lines is subjective.
...
\begin{table}[!h]
  \begin{center}
    \begin{tabular}{lrrr}
      ...
      \midrule
      Todo (4 ops) & 20 & 20 & 5 / 5 \\
      Rooms chat$^{\dagger}$ & ${\sim}50$ & ${\sim}35$ & --- \\
      Social platform (10 microservices, 36 ops) & ${\sim}155$ & ${\sim}245$ & ${\sim}4$ / ${\sim}7$ \\
      CMS (4 tiers, 14 ops) & ${\sim}75$ & ${\sim}80$ & ${\sim}5$ / ${\sim}6$ \\
      \bottomrule
    \end{tabular}
    \caption{Framework-specific code per application. $^{\dagger}$Rooms chat numbers
    are not directly comparable, see discussion below.}\label{loc_table}
  \end{center}
\end{table}

Three patterns emerge from the table. For the Todo application both frameworks use the same
amount of framework code (20 lines, 5 per operation). Kotlin Remote pays 3 lines per remote
function (annotation with context parameter, then the suspend function signature on the next
line, then the function body) while Kotlin RPC pays 2 lines per function (interface signature
plus impl override) plus around 5 lines per service for scaffolding (\kotlini{@Rpc} interface
declaration with braces, impl class declaration with \kotlini{:~ServiceName} and braces). For
one service with four operations these cancel out exactly.

The CMS application has four tiers and fourteen operations and is essentially tied (around 75
versus 80). Kotlin Remote has four hierarchical \kotlini{RemoteConfig} interfaces plus four
session classes, which add up to roughly the same as the four \kotlini{@Rpc} interfaces in
Kotlin RPC. The hierarchical contexts feature does not by itself shrink the line count, the
same information about tiers has to be expressed somewhere. What it changes is where the
information lives: in Kotlin Remote it is in the type system and the compiler enforces it; in
Kotlin RPC the programmer has to track at every call site which stub to use for which call.
Cross-tier internal composition is local in Kotlin Remote (calls dispatch their local bodies
because \kotlini{LocalContext} is a subtype of \kotlini{RemoteContext<X>} for any \kotlini{X}),
and either makes spurious network calls back to localhost or has to step around the service
abstraction in Kotlin RPC.

The social platform shows the biggest gap (around 155 versus 245, Kotlin Remote is about 40
percent less). Per operation that is 4 lines for Kotlin Remote and 7 for Kotlin RPC. Two
things contribute. First, ten microservices in Kotlin RPC mean ten implementation classes that
mostly delegate to repositories (around 100 framework lines that do not exist in Kotlin
Remote). Second, orchestrating microservices like \kotlini{feed} and the BFFs require explicit
stub creation (\kotlini{withService} calls) for every dependent microservice, and matching
\kotlini{registerService} calls inside \kotlini{AllLauncher}. The launchers in Kotlin RPC
contribute around 43 framework lines compared to zero in Kotlin Remote. Kotlin Remote replaces
both with one \kotlini{API.kt} file of top level functions plus one \kotlini{Configs.kt} with
ten \kotlini{RemoteConfig} objects.

It is also worth noticing that Kotlin RPC has a startup ordering constraint that Kotlin Remote
does not. Stub creation in Kotlin RPC opens a websocket connection immediately
(\ki{httpClient.rpc { url(...) }} is a suspend function that connects right away), so a
microservice cannot start before all of its dependencies are already running. The social
platform launcher in Kotlin RPC has to be split into three phases: first start leaf
microservices that have no dependencies, then create stubs to them and start orchestrators,
then create stubs to those and start BFFs. In Kotlin Remote the launcher just starts all ten
servers in any order because remote calls are made on demand inside individual function calls.
...
\kotlini{RemoteContext} values. In Kotlin RPC the central server keeps a per user mailbox and
pushes messages over a server side stream (\kotlini{Flow<ChatMessage>}). Kotlin Remote version
actually has more framework code than Kotlin RPC version (around 50 versus 35) because the
peer-to-peer design needs two \kotlini{RemoteConfig} classes (one per machine role) plus nine
\kotlini{@Remote} functions, against Kotlin RPC version's single \kotlini{@Rpc} interface with
eight methods.

\subsubsection{Per-function overhead}
...
When the function body is a one line delegation to a repository (which is the common case in
our applications) Kotlin RPC overhead is roughly twice the function body. In Kotlin Remote it
is one annotation line plus one context line.
...
In this thesis we developed Kotlin Remote, a lightweight RPC framework specifically for
shared-codebase Kotlin Multiplatform projects. Kotlin Remote uses Kotlin's experimental context
parameters feature to express remote calls. Context parameter of remote function carries
information about target machine, which makes function callable on this machine, and at the same
time appears in function signature, which makes remote calls clearly visible at the type level.
This is a departure from the service-oriented RMI model adopted by gRPC and Kotlin RPC, where
remote calls are hidden behind service interfaces and at the call site look indistinguishable from
local ones.

It is important to restate the scope of the work. Kotlin Remote is designed for shared-codebase
(monolithic) projects in which client and server live in a single Kotlin project, share data
types and are released together. The framework is not meant to be a general purpose RPC solution
and does not try to compete with gRPC, Kotlin RPC or Ktor outside of shared-codebase setting.
Existing RPC frameworks are not boilerplate-heavy in general, their per function overhead is
small. The service interface they require is a legitimate contract between client and server and
is essential for cross-language projects, projects with independently developed client and server
or projects with separately evolving public APIs. In all of these scenarios general purpose
frameworks remain the correct choice. The contribution of this work is showing that in
shared-codebase setting this service interface becomes redundant, because the contract is already
implicit in the shared code, and removing this redundancy gives substantial reduction of code that
the developer has to write.

The goal of the thesis stated in section \ref{goal_and_objectives_section} was to develop an RPC
framework for shared-codebase Kotlin Multiplatform projects that reduces boilerplate code compared
to existing alternatives in this setting. We believe this goal was reached. All four objectives
were completed. We prototyped RPC framework based on context parameters and extended it with
support for distributed objects, that is remote classes with state living on remote machine. We
implemented the framework as Kotlin compiler plugin together with runtime library and tested it.
We evaluated the framework by comparing it with Kotlin RPC on four example applications of
different shape and complexity, all built as shared-codebase projects. On the bigger applications,
like social platform with ten microservices and thirty-six remote operations, Kotlin Remote
requires roughly forty percent less framework-specific code than Kotlin RPC. On smaller
applications the two are close, with Kotlin Remote winning by a small margin on the per function
overhead.

All three technical requirements were met. Any named Kotlin function can be called remotely in
Kotlin Remote, not only methods of service interfaces. This includes top level functions, extension
functions, methods of regular classes and companion object methods. Remote functions are
distinguishable from local ones at the type level by their context parameter of type
\kotlini{RemoteContext}. Kotlin Multiplatform compatibility is preserved: framework does not rely
on reflection and works on all KMP targets (JVM, JavaScript, Native, WebAssembly). All the
metadata needed at runtime is gathered at compile time by the compiler plugin.

Kotlin Remote has limitations that were discussed in section \ref{limitations_section}. The most
important one is the narrow scope of application described in section \ref{narrow_scope_section}.
This narrow scope is intentional and follows from the goal of the thesis. Additionally, the
framework at the moment of writing depends on experimental Kotlin features. Several directions for
future work were identified during development, including per node code slicing, remote lambdas
and bidirectional streaming.

We hope Kotlin Remote will be useful for Kotlin developers working on shared-codebase projects who
want to reduce amount of boilerplate code related to network interactions. For projects outside of
shared-codebase scope existing general purpose frameworks remain the right choice. Source code of
the framework together with example applications is available in a public repository.
...
The idea to use context of the function to determine where it should called comes 
from so called "code coloring" described in Kotlin KEEP. The term code coloring in the KEEP is
not introduced formally, instead only intuition, of what code coloring is, is provided using
examples. Such an intuition cannot be used to reason about "code coloring" in an adequate way.

During work on this thesis better we developed understanding of "code coloring" and 
in this appendix we want to try to give formal definition for it.

Code coloring consists of two aspects: type system and compiler support. The most
clean example of code coloring are suspend functions in Kotlin. Functions with "suspend" keyword 
is basically an extension of Kotlin type system. Suspend function can be called only from other
suspend functions, that gives a shape to the color. But the gist of suspend functions is
how they treated by compiler. Basically suspend function is marking for compiler 
to compile code in a special way, in a CPS style as an automaton. For cps suspend function have 
to receive additional argument (continuation), thus constraints on the form of suspend function I 
mentioned previously. We can say that bodies of remote functions are treated differently from the rest 
of the code. Basically, suspend function can be thought of as a different programming language inside Kotlin,
at least from the point of view of compiler. 

Suspend keyword mechanism that dictates the shape of remote functions is just a ad-hog embodiment of
the abstract concept that can be also expressed by changing function return type or signature (adding additional argument). 
We could return something like deferred, but it would require working with this type in a very explicit fashion, 
which basically eliminates the whole point of coroutines. Good option for return type is monad, because
we can work within monad without thinking of monad's specific type. In case of coroutines reader monad is enough
because we already express callCC semantics using automatons and CPS, thus we don't need monad's feature
for explicit control flow (like in Cont monad). Reader monad can be expressed using context parameters
we used for kotlin remote. 

So basically the problem of code coloring is the problem of embedded languages. Because we want to embed
language with its own compilation logic in host language. We are concentrating on embedding functions
because it's a good separate part of code. With coroutines and Kotlin Remote language semantics
inside suspend or remote function can be fully expressed by rewriting code by the compiler.
Thus one more restriction on code coloring: function bodies of embedded languages should be compiled to
host language, in our case kotlin. To make it all easier we used kotlin syntax even for embedded language.
However, in common case it is not required. With kotlin it is required because there is no way to change
parsing pass in kotlin compiler, we can only translate one syntactycally correct code to other.
...
At last definition for code color can be given. Code color is a set of functions which bodies are translated (from host language to host language) during compilation in a special way, 
described by the color. Color consists of translator and a way to mark colored set of functions. Colors
applies restriction either on from what functions colored function can be called,
what function can be called from colored ones or both. That depends on the color type.
...
code coloring is coeffect system with metaprogramming on function level.
