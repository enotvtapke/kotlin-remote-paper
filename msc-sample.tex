...
is target machine of this remote call, machine where call is actually executing.

\subsubsection{Client} \label{client-section}

Now let us take a look at how remote call is done on the client in Kotlin Remote. In order to call
...
using stub. To better understand what stub is and how provides necessary information for remote
...
There is less functional alternative to Reader monad that is called \term{implicit parameters}.
These are parameters of the function that are resolved from the function invocation context and
should not be passed to explicitly when function is called in contrast with normal parameters. New
...
to \kotlini{multiply}, like on figure \ref{multiply_client_context}. It is inconvenient to write
\kotlini{c.client.call(...)} for every remote function. This problem will be addressed later when
code generation done by the framework will be discussed.
...
Here remote client contains all the information needed for remote call, like target address and
...
\term{kotlinx.serialization} \cite{noauthor_kotlinkotlinxserialization_2026}. This
...
which is called and \kotlini{arguments} is arguments with which this function was called. This
class has a custom serializer that serializes \kotlini{arguments: Array<Any?>} using
\kotlini{callableName}. This serializer is created by \kotlini{remoteSerializersModule}. But it is
not a magic. To implement this custom serializer information about remote function with name
\kotlini{callableName} is needed. The only way to get this information for Kotlin Multiplatform 
(\term{KMP}) is by asking user to provide it or gathering it automatically somehow. The problem is this 
information cannot be gathered in runtime because KMP (unlike Java or Kotlin JVM backend) does not
support fully fledged reflection. This means that we cannot get function signature in runtime or type 
by its name, etc. But this is exactly information needed for serialization. For now we will think that
required information is gathered somehow and stored inside structure called \kotlini{CallableMap}. 
This is basically a map from fully qualified names of remote function 
to information about this function which is represented by \kotlini{RemoteCallable} from figure 
\ref{remote_call_and_callable_class}.
...
\subsection{Remote functions code generation}
...
been discussing \ki{core-functions} module that contains logic related to remote functions.
\ki{core-classes} is independent of \ki{core-functions} and will be discussed later. Now we briefly
discuss \ki{core-ktor} module which contains Kotlin Remote integration with Ktor. Ktor is a mature framework
that offers lots of functions like auth support, logging etc. Kotlin Remote used together with Ktor
supports the same functionality Ktor does. For example, authentication can be supported using code from 
listing \ref{auth_example}. Notice call to \ki{handleRemoteCall} function in \ki{server}. This function
passes control to \ki{core-ktor} module that deserializes arguments of remote call, invokes remote function,
serializes and sends result to the client.
...
All these problems are not new and were addressed in lots of existing RMI framework, like Java RMI. 
...
custom \term{kotlinx.serialization} serializer is generated. 

This custom serializer serializes remote class as stub. Stub is a subclass of remote class that 
is generated for every remote class. Stub only has ID and URL of the machine it was created on. During
serialization original class is added to the special map called \ki{RemoteInstancesPool}. 
...
serialized on, or as just stub. To understand whether stub was serialized on a current machine, URL
of the current machine is compared with URL from the stub.

This way "remote references" are supported by Kotlin Remote without altering logic related to remote function.
Only serialization functionality is changed. Because of that logic related to remote classes lives
in a separate Gradle module named \texttt{core-classes} which neither depends nor is dependent on 
\texttt{core-functions} module described earlier.   


\subsection{Garbage collection}

For now there is no way to remove remote classes from \ki{RemoteInstancesPool}. This is a serious resource
leak. To resolve this issue leasing is used. Each time stub is deserialized as stub current machine 
begins to send lease requests to machine where original class of the stub is stored. When machine with original
class does not get lease requests for a long time it removes class from \ki{RemoteInstancesPool} and
then it is garbage collected. Machine possessing stub stops sending lease requests when stub is garbage collected
on it. To implement that weak references are used.
