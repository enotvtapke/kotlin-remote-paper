...
works especially well when server and client share one codebase, which is highly important for 
KMP environment.
...
Investigate how coeffects can be used for remote procedure calls

Investigate whether RPC in the shared-codebase setting can be expressed through coeffects using 
Kotlin's experimental context parameters feature, instead of the service-oriented model that gRPC and Kotlin RPC adopt, and assess where this approach is preferable and where it is not.
...
rewritten using Kotlin RPC. All implementations are functionally equivalent, the only difference
is in the way network interactions are organized. As we are interested in network code, we will
be focusing on it during comparison. Business logic of applications (database access, command
line user interface, etc.) is roughly the same in all implementations. Source code is available
at \texttt{integration-tests/} folder of Kotlin Remote repository.

We compare Kotlin Remote only with Kotlin RPC. The reason is that for Kotlin only and transport
agnostic setting, which is the setting Kotlin Remote targets, gRPC and Kotlin RPC are very close
in features: both rely on service interface declaration, both generate stubs and server bases at
...
mapping. We did prototype gRPC versions for the smaller applications and the per function
overhead in gRPC was always the largest of the three frameworks. So we drop gRPC from the main
comparison and focus on the closer alternative.
...
per microservice overhead and orchestration. \term{CMS} is a content management system with
hierarchical context tiers and Ktor authentication, designed to exercise the hierarchical
contexts feature.
...
\subsubsection{Narrow scope of application}

Kotlin Remote is optimized to work in a very specific environment: Kotlin only shared codebase (monolithic)
projects. It restricts its usability by a very limited number of projects in contrast to Kotlin RPC and
especially gRPC. This is a drawback inherent in the chosen approach and is dictated by the goal
of this work. However, we still believe that it is worth mentioning this limitation explicitly. 
...
of the code. Basically, suspend function can be thought of different programming language inside kotlin,
...
the abstract concept that can be fully expressed changing function return type or signature (adding additional argument). 
...
we can work within monad without thinking of monads specific type. In this case reader monad is enough
because we already express callCC semantics using automatons and CPS, thus we don't need monad's feature
to for explicit control flow (like in Cont monad). Reader monad can be expressed using context parameters
we used for kotlin remote. 

So basically the problem of code coloring is the problem of embedded languages. Because we want to embedd
...
To give shape to colors using type we basically use coeffects as our theoretycal basis.
In Kotlin common functions are not pure coeffect free at all, they require a lot from theyr evnvironment,
like a way to run threads. So new "colors" can be introduced to be more restrictive than 
...
Code color is a set of functions which bodies are translated during compilation in a special way, 
...
are metaprogramming on function level (like rust procedural macros).
