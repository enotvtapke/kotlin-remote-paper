...
A large subset of modern software uses network interactions extensively. One of the languages that is
actively used to develop such software is Kotlin. However, standard for Kotlin technologies like
REST for organising network interactions are overly explicit and require writing a lot of
additional code by applying specific templates. Such code is usually called \term{boilerplate code}.

In this thesis, an alternative mechanism for organising network interactions in Kotlin was developed.
This mechanism uses approach uncommon for Kotlin, called Remote Procedure Call (RPC). This approach
allows the invocation of Kotlin function on a remote machine, provided the function is annotated in a
special way. Such annotated functions are called \term{remote functions}.

The novelty of the mechanism developed in this work lies in the usage of coeffects. In contrast
with traditional RPC, mechanism developed in this work uses context in which remote function is
invoked to determine on what machine function should be executed. This leads to more implicit
organisation of network interactions and as a result greatly reduces boilerplate code required for
