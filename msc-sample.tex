...
identified. Some of them are related to the early stage of the framework, others are inherent
to the chosen approach.

\subsubsection{Verbose Ktor wiring}

Kotlin Remote requires more verbose Ktor wiring than Kotlin RPC. On the server side user has to
install \kotlini{ContentNegotiation} with serializer module built from \ki{genCallableMap()}, in
addition to installing \kotlini{KRemote} plugin and registering remote routes. On the client
side \kotlini{ContentNegotiation} has to be installed in \kotlini{HttpClient} with the same
serializer module. Kotlin RPC hides equivalent work behind \kotlini{install(Krpc)} on the
server and \kotlini{installKrpc()} on the client. This Ktor wiring is not part of our
framework code comparison in table \ref{loc_table}, but the user still has to write it.
\kotlini{RemoteClient.kt} and \kotlini{RemoteServer.kt} files in all our example applications
are almost identical and could probably be hidden behind a more opinionated default.
...
way to return a stream of values from remote function or to push values from server to client.
As shown in the rooms chat application, this can be worked around using peer-to-peer remote
calls, but this is not always desirable because it requires running an embedded server on every
client. Kotlin RPC supports streaming through Kotlin \kotlini{Flow}. Adding similar support to
Kotlin Remote is one of the directions for future work.
...
If user renames remote function, moves it to another package or changes its parameter types,
all existing clients break. There is no versioning story like in protobuf where fields have
stable numbers and can be renamed without breaking the wire format. For applications where
client and server are in the same codebase this is not really a problem because they are always
built together. For applications where client and server have separate release cycles this can
...
For polyglot environments gRPC is still a better choice.
...
Looking at \kotlini{RemoteClient.kt} and \kotlini{RemoteServer.kt} of any example application
one can notice that \kotlini{genCallableMap} is called several times. Each call is replaced at
compile time with the same map literal which can have hundreds of entries. As a result generated
code is larger than necessary. Caching of the result on user side is possible but inconvenient
because \kotlini{genCallableMap} has to be called from different scopes. Compiler plugin could
