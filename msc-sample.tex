...
Apart from looking at existing RPC frameworks, this section also takes a look on related ideas
from functional programming. Namely on \term{coeffects}, that are foundational concept on which
Kotlin Remote is built. Coeffects are tightly related to \term{implicit parameters}, which Kotlin
recently got an implementation of in form of experimental \term{context parameters} feature. Both
of these ideas will be discussed below.
...
Most well-known classic implementations of RMI are CORBA, Java RMI \cite{noauthor_java_nodate} and
...
interactions completely from the developer \cite{ryan_grpc_2015}.
...
Reader monad is famously used by RPC framework called Servant \cite{noauthor_servant_nodate} for
...
body is executed locally on the server). It is worth noticing that other languages also explored
similar ideas. For example, Opa \cite{noauthor_opa_nodate} is a language designed for web
applications in which type system can express where particular piece of code is executed (client or
server). This is essentially the same coeffect we use in Kotlin Remote, but baked directly into the
language.
...
Remote to be suspend.

Internally coroutines are implemented using continuations. Suspend functions are transformed by the
Kotlin compiler into state machines that take continuation as an additional parameter. Currently
Kotlin coroutines support only single-shot continuations, although there is an ongoing discussion
about adding multi-shot continuations
\cite{noauthor_keepnotes0004-multishot-continuationsmd_nodate}.
