...
Table \ref{loc_table} shows the size of network code in lines for each application. By network
code we mean code that exists only because the application is distributed: remote function
declarations and decorations, service interfaces, client and server setup. Pure business logic
(repositories, command line interface, desktop UI) and pure data classes are excluded. Numbers
should be taken as rough approximations because in Kotlin Remote network annotations are
interleaved with business logic and the exact split is somewhat subjective.
...
\begin{table}[!h]
  \begin{center}
    \begin{tabular}{lrrr}
      ...
      \midrule
      Todo (4 ops) & 164 & 86 & 41 / 22 \\
      Rooms chat$^{\dagger}$ & ${\sim}165$ & ${\sim}120$ & --- \\
      Social platform (10 microservices, 36 ops) & 545 & 864 & 15 / 24 \\
      CMS (4 tiers, 14 ops) & 336 & 341 & 24 / 24 \\
      \bottomrule
    \end{tabular}
...
The table tells three different stories at three different scales. For the smallest application
(Todo) Kotlin Remote produces noticeably more network code than Kotlin RPC. The reason is per
application setup of \kotlini{RemoteClient} and \kotlini{RemoteServer} which is around 100 lines
for any application regardless of its size. Kotlin RPC has a leaner per application setup. For
small applications with few operations this setup cost dominates.

For social platform with ten microservices and thirty-six operations the situation reverses.
Kotlin Remote needs 545 lines, Kotlin RPC needs 864. Per operation that is 15 versus 24 lines.
The per application setup cost in Kotlin Remote is the same as for Todo, but it is now
amortized over many operations. Kotlin RPC on the other hand pays per microservice cost in
three places: \kotlini{@Rpc} interface declaration, implementation class (most impl methods are
one line delegations to a repository) and a launcher that registers the service. Orchestrating
microservices like \kotlini{feed} and the BFFs pay additional cost because they take downstream
service stubs as constructor parameters, and the launcher has to create those stubs before
starting the server. The client side has the same cost: every microservice the client wants to
talk to needs a separate stub. In Kotlin Remote each dependency is a context parameter resolved
at the call site, no stub plumbing is involved anywhere.
...
For CMS the lines of code are essentially tied (336 versus 341). The hierarchical contexts
feature does not by itself reduce the line count, the same information about tiers has to be
expressed somewhere. What changes is where the information lives. In Kotlin Remote the
hierarchy is in the type system and is enforced automatically by the compiler. In Kotlin RPC
the hierarchy lives in the project structure and the programmer has to track at every call
site which stub to use for which call. Cross-tier internal composition is local in Kotlin
Remote and either makes spurious network calls back to localhost or has to step around the
service abstraction in Kotlin RPC.
...
as a remote call from the central server to the user machine. In Kotlin RPC the central server
keeps a per user mailbox and pushes messages over a server side stream
(\kotlini{Flow<ChatMessage>}). Kotlin Remote version effectively does more (every participant
is both client and server), this shows up in the line count. The opposite reading is also
valid: Kotlin Remote was forced into the peer-to-peer design because it does not support
streaming (see section \ref{streaming_limitation_section}), and the extra lines are the cost of
...
Kotlin Remote requires per application setup code that is roughly the same from one application
to another. \kotlini{RemoteClient.kt} and \kotlini{RemoteServer.kt} files in all our example
applications are almost identical, around 100 lines together. This code configures
\kotlini{HttpClient}, \kotlini{ContentNegotiation} plugin, serialization modules for remote
functions and remote classes, installs \kotlini{KRemote} plugin and registers Ktor routes. Most
of it is parameter passing between framework components and could probably be hidden behind a
more opinionated default. For applications with few operations this overhead dominates the
