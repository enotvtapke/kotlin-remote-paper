...
produce exactly the same number of framework-specific lines (25 versus 25). The reason
...
the costs cancel almost line for line: Kotlin Remote pays around three lines per
function (the \kotlini{@Remote} annotation, the \kotlini{context(...)} declaration and
the suspend function signature) plus a small \kotlini{RemoteConfig} object, while Kotlin
RPC pays roughly two lines per function spread across an \kotlini{@Rpc} interface and a
matching implementation class plus a few lines of fixed scaffolding for those two types.
With only four remote operations there is not enough material for either pattern to
compound. The UI side of the comparison is identical: both clients open one
service handle at startup and make nine direct remote calls from inside their UI
handlers. We therefore read this as a baseline result, not a win or a loss for either
...
line count (25 vs 25): the application has too little network code for the structural
