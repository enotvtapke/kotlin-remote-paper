...
\begin{center}
  \Large
  Master Thesis in Advanced Software Technology
\end{center}
\vfill
\begin{table}[h]
\large
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}p{7cm}rl}
  Submission: May 19, 2026 & Supervisor: & \mysupervisor \\
  & Second reviewer: & Dr. D. Berezun
\end{tabular*}
\end{table}
\rule{\textwidth}{1pt}
\begin{center}
...
During the preparation of this master's thesis, artificial intelligence tools were utilized as
supplementary aids in the writing process. Specifically, AI was used to generate illustrative
examples and to assist in drafting descriptions for certain sections of the research and
methodology to enhance clarity. All AI-assisted content was thoroughly reviewed, critically
evaluated, and significantly edited by the author to ensure accuracy, originality, and alignment
...
require developer to describe remote
...
fail. Existing approaches follows a strict OOP style which is not the only viable option for Kotlin. These
...
ordinary function syntax. Kotlin Remote uses similar design: where Servant uses the Reader monad,
Kotlin Remote uses a context parameter; where Servant has \mintinline{haskell}{runClientM}, Kotlin
Remote has the \kotlini{context(config) { ... }} block.
...
It is worth noticing that \ki{genCallableMap} adds every \ki{@Remote} function to the callable
map regardless of its Kotlin visibility, including functions with non-public visibility. This
means that by using \ki{genCallableMap} it is possible to invoke such functions over the
network, so the \ki{@Remote} annotation, rather than the Kotlin visibility modifier, is the
real boundary of the server's network API. We recommend keeping every \ki{@Remote} function
\kotlini{public} so that Kotlin visibility matches the actual network exposure. A frontend
compiler plugin check that forbids lower-than-\kotlini{public} visibility on \ki{@Remote}
declarations would make this convention enforceable mechanically. We did not implement such a
check because the planned remote lambdas extension (section \ref{future_work_section}) will
need to dispatch closure-captured callables that are not public top-level Kotlin functions, so
a strict visibility rule would have to be revisited together with that work.
...
\begin{table}[!h]
	\begin{center}
		\begin{tabular}{lccc}
			...
			Remoteness marked at call site & No             & No                   & enclosing \kotlini{context} \\
...
\texttt{./gradlew lineReports}, from inside \texttt{integration-tests} directory compiles every measured module with the
...
The operational rule for what counts as framework-specific is symmetric between the two
frameworks: a line is counted only if it would not be present in an equivalent in-process
Kotlin program that did not use any RPC framework but expressed the same business logic.
Concretely, on the Kotlin Remote side this admits the \kotlini{@Remote} annotation, the
\kotlini{context(_: RemoteContext<...>)} parameter clause, \kotlini{context(...) \{ ... \}}
and \kotlini{with(...) \{ ... \}} blocks whose argument is a \kotlini{RemoteContext} or
\kotlini{RemoteConfig}, \kotlini{.asContext()} and \kotlini{runWith \{ ... \}} calls, and
\kotlini{RemoteConfig} subtype declarations. On the Kotlin RPC side it admits the
\kotlini{@Rpc} annotation, the entire \kotlini{@Rpc} interface (declaration, every method
signature, closing brace), the impl class shell (declaration line through the opening brace
plus the closing brace), and \kotlini{withService<...>} / \kotlini{registerService<...>}
call sites. The pass deliberately does \emph{not} count the \kotlini{suspend fun} signature
of a \kotlini{@Remote} function, the \kotlini{override} signature lines of methods inside
\kotlini{@Rpc} impl classes, the bodies of either kind of method, or call sites that invoke
a remote function (\kotlini{service.foo()} on a Kotlin RPC stub or a bare \kotlini{foo()}
inside a Kotlin Remote \kotlini{context(...) \{ ... \}} block), because those lines describe
business logic and would be present in a non-network version of the same project.
...
\begin{table}[!h]
  \begin{center}
    \begin{tabular}{lrrr}
      ...
      \midrule
      Todo (4 ops)                    &  12 &  12 & ${\sim}$ 3 / 3 \\
      Social platform (36 remote ops) &  89 & 136 & ${\sim}$ 2 / 4 \\
      CMS (14 remote ops)             &  44 &  55 & ${\sim}$ 3 / 4 \\
      \bottomrule
    \end{tabular}
    \caption{Framework-specific code per application. Rooms chat is excluded; see discussion below.}\label{loc_table}
  \end{center}
\end{table}

Three patterns emerge from the table. For the Todo application the two frameworks
produce exactly the same number of framework-specific lines (12 versus 12). The reason
is that there is simply too little network code in the application for the structural
differences between Kotlin Remote and Kotlin RPC to register as a gap. Todo exposes
four remote functions consumed from one desktop UI through one server, and on each side
the costs cancel almost line for line: Kotlin Remote pays two marker lines per remote
function (\kotlini{@Remote} annotation and \kotlini{context(_: RemoteContext<...>)}
clause) plus a small \kotlini{RemoteConfig} object, while Kotlin RPC pays roughly one
line per function inside the \kotlini{@Rpc} interface plus the fixed scaffolding of one
interface declaration and one matching implementation class shell. With only four remote
operations there is not enough material for either pattern to compound. The UI side of
the comparison contributes a single line per side as well: a \kotlini{withService<...>}
call in the Kotlin RPC client and a \kotlini{runWith \{ ... \}} call in the Kotlin Remote
client. We therefore read this as a baseline result, not a win or a loss for either
framework: very small APIs are too small to choose a framework over.

The CMS application has four tiers and fourteen remote operations. Kotlin Remote uses
about 20\% fewer framework-specific lines than Kotlin RPC (44 versus 55), and the
difference is concentrated in the structural scaffolding that Kotlin RPC requires for
each tier: four \kotlini{@Rpc} interface declarations and four implementation class
shells, none of which have a counterpart in the Kotlin Remote version where the fourteen
operations live as top-level \kotlini{@Remote} functions and four short
\kotlini{RemoteConfig} subtypes carry the tier identity in the type system. Hierarchical
contexts also pay off here: the Kotlin RPC version has to obtain a separate stub for
each tier the client wants to call, while in Kotlin Remote any session that satisfies a
higher-tier configuration also satisfies all lower tiers via subtyping. Cross-tier
internal composition is local in Kotlin Remote (calls dispatch their local bodies because
\kotlini{LocalContext} is a subtype of \kotlini{RemoteContext<X>} for any \kotlini{X}),
and either makes spurious network calls back to localhost or has to step around the
service abstraction in Kotlin RPC.

The social platform shows the largest absolute gap (89 versus 136, Kotlin Remote is
about 35\% shorter). Per operation that is roughly two lines for Kotlin Remote and four
for Kotlin RPC. Two things contribute. First, ten microservices in Kotlin RPC mean ten
\kotlini{@Rpc} interfaces and ten implementation class shells, none of which have a
counterpart on the Kotlin Remote side where the same thirty-six operations are top-level
\kotlini{@Remote} functions in a single \kotlini{API.kt}. Second, orchestrating
microservices like \kotlini{feed} and the BFFs require explicit stub creation
(\kotlini{withService} calls) for every dependent microservice, and matching
\kotlini{registerService} calls inside \kotlini{AllLauncher}. Kotlin Remote replaces both
with one \kotlini{Configs.kt} file containing ten \kotlini{RemoteConfig} objects, picked
out at the call site by writing a \kotlini{context(SomeServiceConfig.asContext()) \{ ... \}}
block.
...
approximately 35\% and 20\% respectively compared to Kotlin RPC. On the smallest application,
Todo (four remote functions invoked from a desktop UI), the two frameworks tie at the same
line count (12 vs 12): the application has too little network code for the structural
differences between the two frameworks to compound into a measurable gap. Rooms chat is not
directly comparable because the two implementations make architecturally different choices
(peer-to-peer remote calls vs server-pushed \kotlini{Flow}). The honest summary is that
Kotlin Remote reduces boilerplate once the network surface of an application grows beyond
a handful of operations on a single service: the difference between paying per remote
function (Kotlin Remote) and paying per service interface plus implementation class
(Kotlin RPC) only shows up when there are several services or tiers to express. On a tiny
API the two frameworks are interchangeable in size; on a project with multiple
microservices or hierarchical tiers Kotlin Remote wins. The framework-overhead benchmarks
of section~\ref{per_call_latency_section} show that this boilerplate reduction does not
come at the cost of runtime performance. Local dispatch of a
...
Kotlin Remote has limitations discussed in section \ref{limitations_section}. The most important
one is the narrow scope of application described in section \ref{narrow_scope_section}. The
framework also depends on experimental Kotlin features. Several directions for future work were
identified during development, including per node code slicing, remote lambdas and bidirectional
streaming.
