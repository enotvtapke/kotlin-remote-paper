...
Kotlin Remote is designed for shared-codebase (monolithic) projects in which client and server live
in a single Kotlin project, share data types and are released together. The framework is not meant
to be a general purpose RPC solution and does not try to compete with gRPC, Kotlin RPC or Ktor
outside of shared-codebase setting. Existing RPC frameworks are not boilerplate-heavy in general,
their per function overhead is small. The service interface they require is a legitimate contract
between client and server and is essential for cross-language projects, projects with independently
developed client and server or projects with separately evolving public APIs. In all of these
scenarios general purpose frameworks remain the correct choice. The contribution of this work is
showing that in shared-codebase setting this service interface becomes redundant, because the
contract is already implicit in the shared code, and removing this redundancy gives substantial
reduction of code that the developer has to write. 
...
to existing alternatives in this setting. We believe this goal was reached.
