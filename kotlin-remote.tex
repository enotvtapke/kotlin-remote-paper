...
systems are typical examples. Many such projects follow \term{shared-codebase} model in which client and server live in a single Kotlin project and share
...
applications and distributed systems. Many of these projects follow \term{shared-codebase} model in
which client and server live in a single Kotlin project and share data types and parts of business
logic. By shared-codebase project we mean a project where all the parts that communicate over the
...
from the world of functional programming that captures what a computation requires from its
...
The approach used in Kotlin Remote framework is heavily inspired by ideas from the world of
...
the Haskell programming language to implement remote calls. Figure \ref{servant_remote_call}
...
Let us imagine we want to execute some function \kotlini{multiply} remotely. Here is the client
implementation for it on figure \ref{multiply_client}.
...
Now \kotlini{RemoteInvokator} should be changed a bit to reflect this duality of remote functions.
...
Another quirk about \ki{genCallableMap} is that it will not generate entries from remote
functions from dependencies, because source code of dependencies is not accessible (it is
already compiled). To alleviate that, dependencies can save their callable maps generated by
\ki{genCallableMap}, for example, in a public static property. This way dependent code can use
these saved callable maps by, for example, merging them with its own callable map.
...
site, neither supports stateful remote objects with framework-managed lifecycle, neither
expresses hierarchies between services in the type system. They differ on cross-language support
and on whether the transport is fixed. Kotlin Remote differs from both alternatives on the API
style rows and on the absence of streaming, which it currently does not support. The
...
A small social network backend split into 10 microservices: \kotlini{users}, \kotlini{posts},
...
Helper \kotlini{dep<T>()} in the listing is a thin wrapper over Koin
...
\subsubsection{Encapsulation and exposure surface}\label{encapsulation_limitation_section}

The \ki{genCallableMap} intrinsic described in section \ref{callable_map_generation} collects
every function in the compilation unit annotated with \ki{@Remote} into the map that the server
uses to dispatch incoming calls. The collection step does not filter by Kotlin visibility
modifiers, so a function annotated with \ki{@Remote} becomes callable over the network even if
its Kotlin visibility is lower than \kotlini{public}. This breaks the usual expectation that
lower visibility implies a smaller exposure surface: as soon as a function is annotated, it is
part of the public network API of the server regardless of how it is declared in Kotlin source.

The actual gate is the \ki{@Remote} annotation itself rather than Kotlin visibility. This gate
is currently a single-bit decision (annotated or not) without any finer-grained access control.
In particular, the framework does not provide a built-in mechanism to restrict which clients
can call which remote functions; the CMS example in section \ref{cms_app_section} uses Ktor's
basic authentication and HTTP routing for that, but this is enforced by Ktor configuration on
top of Kotlin Remote, not by the framework itself. Production deployments must therefore treat
every \ki{@Remote} function as part of the public surface of the server and apply transport-level
authentication and authorization explicitly.

A natural mitigation is for the user to keep every \ki{@Remote} function \kotlini{public}, so
that Kotlin visibility matches the runtime exposure, and to reserve unexposed helpers for
non-annotated functions; we recommend this convention. A more rigorous solution would be for the
frontend compiler plugin to either forbid lower-than-\kotlini{public} visibility on \ki{@Remote}
declarations, or to integrate with a per-function authorization scheme. Both are left to future
work.
